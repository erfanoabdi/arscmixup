 ...
 */
    ...
    do {
...
#ifdef DEBUG_ALL
            QUP_LOGI("+++ reading %ld bytes (%ld left)\n",
                    getSize, compRemaining);
#endif

            int cc = TEMP_FAILURE_RETRY(read(fd, readBuf, getSize));
            if (cc < 0) {
                QUP_LOGI("inflate read failed: %s", strerror(errno));
            } else if (cc != (int) getSize) {
                QUP_LOGI("inflate read failed (%d vs %ld)", cc, getSize);
                goto z_bail;
            }
        ...
        if (zerr != Z_OK && zerr != Z_STREAM_END) {
            QUP_LOGI("zlib inflate call failed (zerr=%d)\n", zerr);
            goto z_bail;
        }
    ...
    if ((long) zstream.total_out != uncompressedLen) {
        QUP_LOGI("Size mismatch on inflated file (%ld vs %ld)\n",
 ...
 */
    ...
    do {
        ...
        if (zstream.avail_in == 0) {
            ...
            if (cc != (int) getSize) {
                QUP_LOGI("inflate read failed (%d vs %ld)\n",
                        cc, getSize);
                goto z_bail;
            }
