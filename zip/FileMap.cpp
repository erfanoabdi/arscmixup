...
FileMap::~FileMap(void)
{
    ...
    if (mBasePtr && munmap(mBasePtr, mBaseLength) != 0) {
        QUP_LOGI("munmap(%p, %d) failed\n", mBasePtr, (int) mBaseLength);
    }
#endif
#ifdef HAVE_WIN32_FILEMAP
    if (mBasePtr && UnmapViewOfFile(mBasePtr) == 0) {
        QUP_LOGI("UnmapViewOfFile(%p) failed, error = %ld\n", mBasePtr,
              GetLastError() );
    }
        ...
        bool readOnly)
{
    ...
    if (mFileMapping == NULL) {
        QUP_LOGI("CreateFileMapping(%p, %lx) failed with error %ld\n",
    ...
    mBasePtr = MapViewOfFile( mFileMapping, 
                              ...
                              adjLength );
    if (mBasePtr == NULL) {
        QUP_LOGI("MapViewOfFile(%ld, %ld) failed with error %ld\n",
    ...
    if (mPageSize == -1) {
        ...
        if (mPageSize == -1) {
            QUP_LOGI("could not get _SC_PAGESIZE\n");
            return false;
        }
#else
...
try_again:
    ...
    QUP_LOGI("MAP: base %p/%d data %p/%d\n",
