...
int ZipFileRO::entryToIndex(const ZipEntryRO entry) const
{
    ...
    if (ent < 0 || ent >= mHashTableSize || mHashTable[ent].name == NULL) {
        QUP_LOGI("Invalid ZipEntryRO %p (%ld)\n", entry, ent);
        return -1;
    }
    return ent;
}
...
status_t ZipFileRO::open(const char* zipFileName)
{
    ...
    if (fd < 0) {
        QUP_LOGI("Unable to open zip '%s': %s\n", zipFileName, strerror(errno));
...
bool ZipFileRO::mapCentralDirectory(void)
{
    ...
    if (readAmount < kEOCDSize) {
        QUP_LOGI("File too short to be a zip file");
        return false;
    }

    unsigned char* scanBuf = (unsigned char*) malloc(readAmount);
    if (scanBuf == NULL) {
        QUP_LOGI("couldn't allocate scanBuf: %s", strerror(errno));
        free(scanBuf);
        return false;
    }
    ...
    if (lseek64(mFd, 0, SEEK_SET) != 0) {
        QUP_LOGI("seek to start failed: %s", strerror(errno));
        free(scanBuf);
        return false;
    }
    ...
    if (actual != (ssize_t) sizeof(int32_t)) {
        QUP_LOGI("couldn't read first signature from zip archive: %s", strerror(errno));
        free(scanBuf);
        return false;
    }
    ...
    if (header != kLFHSignature) {
        QUP_LOGI("Not a Zip archive (found 0x%08x)\n", header);
        free(scanBuf);
        return false;
    }
    ...
    if (lseek64(mFd, searchStart, SEEK_SET) != searchStart) {
        QUP_LOGI("seek %ld failed: %s\n",  (long) searchStart, strerror(errno));
        free(scanBuf);
        return false;
    }
    actual = TEMP_FAILURE_RETRY(read(mFd, scanBuf, readAmount));
    if (actual != (ssize_t) readAmount) {
        QUP_LOGI("Zip: read " ZD ", expected " ZD ". Failed: %s\n",
    ...
    for (i = readAmount - kEOCDLen; i >= 0; i--) {
        if (scanBuf[i] == 0x50 && get4LE(&scanBuf[i]) == kEOCDSignature) {
            QUP_LOGI("+++ Found EOCD at buf+%d\n", i);
            break;
        }
    }
    if (i < 0) {
        QUP_LOGI("Zip: EOCD not found, %s is not zip\n", mFileName);
        free(scanBuf);
        return false;
    }
    ...
    if ((commentSize > (mFileLength - kEOCDLen))
        ...
        QUP_LOGI("comment size runs off end of file");
        return false;
    }

    QUP_LOGI("+++ numEntries=%d dirSize=%d dirOffset=%d\n",
        numEntries, centralDirSize, centralDirOffset);

    mDirectoryMap = new FileMap();
    if (mDirectoryMap == NULL) {
        QUP_LOGI("Unable to create directory map: %s", strerror(errno));
        return false;
    }

    if (!mDirectoryMap->create(mFileName, mFd, centralDirOffset, centralDirSize, true)) {
        QUP_LOGI("Unable to map '%s' (" ZD " to " ZD "): %s\n", mFileName,
...
bool ZipFileRO::parseZipArchive(void)
{
    ...
    for (int i = 0; i < numEntries; i++) {
        if (get4LE(ptr) != kCDESignature) {
            QUP_LOGI("Missed a central dir sig (at %d)\n", i);
            goto bail;
        }
        if (ptr + kCDELen > cdPtr + cdLength) {
            QUP_LOGI("Ran off the end (at %d)\n", i);
            goto bail;
        }

        long localHdrOffset = (long) get4LE(ptr + kCDELocalOffset);
        if (localHdrOffset >= mDirectoryOffset) {
            QUP_LOGI("bad LFH offset %ld at entry %d\n", localHdrOffset, i);
            goto bail;
        }

        unsigned int gpbf = get2LE(ptr + kCDEGPBFlags);
        if ((gpbf & kGPFUnsupportedMask) != 0) {
            QUP_LOGI("Invalid General Purpose Bit Flag: %d", gpbf);
            goto bail;
        }
        ...
        if (memchr(name, 0, nameLen) != NULL) {
            QUP_LOGI("Filename contains NUL byte");
            goto bail;
        }
        ...
        if ((size_t)(ptr - cdPtr) > cdLength) {
            QUP_LOGI("bad CD advance (%d vs " ZD ") at entry %d\n",
                (int) (ptr - cdPtr), (ZD_TYPE) cdLength, i);
            goto bail;
        }
    }
    QUP_LOGI("+++ zip good scan %d entries\n", numEntries);
    result = true;
    ...
    size_t* pCompLen, off64_t* pOffset, long* pModWhen, long* pCrc32) const
{
    ...
    if (pOffset != NULL) {
        ...
        if (localHdrOffset + kLFHLen >= cdOffset) {
            QUP_LOGI("ERROR: bad local hdr offset in zip\n");
            return false;
        }
        ...
        if (actual != sizeof(lfhBuf)) {
            QUP_LOGI("failed reading lfh from offset %ld\n", localHdrOffset);
            return false;
        }

        if (get4LE(lfhBuf) != kLFHSignature) {
            QUP_LOGI("didn't find signature at start of lfh; wanted: offset=%ld data=0x%08x; "
                    "got: data=0x%08lx\n",
...

        {
            ...
            if (lseek64(mFd, localHdrOffset, SEEK_SET) != localHdrOffset) {
                QUP_LOGI("failed seeking to lfh at offset %ld\n", localHdrOffset);
                return false;
            }
        ...
        if (dataOffset >= cdOffset) {
            QUP_LOGI("bad data offset %ld in zip\n", (long) dataOffset);
            return false;
        }

        /* check lengths */
        if ((dataOffset >= cdOffset) || (compLen > (cdOffset - dataOffset))) {
            QUP_LOGI("bad compressed length in zip (%ld + " ZD " > %ld)\n",
        ...
        {
            QUP_LOGI("ERROR: bad uncompressed length in zip (%ld + " ZD " > %ld)\n",
...
bool ZipFileRO::uncompressEntry(ZipEntryRO entry, int fd) const
{
    ...
    if (method == kCompressStored) {
        ...
        if (actual < 0) {
            QUP_LOGI("Write failed: %s\n", strerror(errno));
            goto unmap;
        } else if ((size_t) actual != uncompLen) {
            QUP_LOGI("Partial write during uncompress (" ZD " of " ZD ")\n",
                (ZD_TYPE) actual, (ZD_TYPE) uncompLen);
            goto unmap;
        } else {
            QUP_LOGI("+++ successful write\n");
        }
    } else {
    ...
    size_t uncompLen, size_t compLen)
{
    ...
    if (zerr != Z_OK) {
        if (zerr == Z_VERSION_ERROR) {
            QUP_LOGI("Installed zlib is not compatible with linked version (%s)\n",
                ZLIB_VERSION);
        } else {
            QUP_LOGI("Call to inflateInit2 failed (zerr=%d)\n", zerr);
        }
        goto bail;
    }
    ...
    if (zerr != Z_STREAM_END) {
        QUP_LOGI("Zip inflate failed, zerr=%d (nIn=%p aIn=%u nOut=%p aOut=%u)\n",
    ...
    if (zstream.total_out != uncompLen) {
        QUP_LOGI("Size mismatch on inflated file (%ld vs " ZD ")\n",
    ...
    size_t uncompLen, size_t compLen)
{
    ...
    do {
        ...
        if (zerr != Z_OK && zerr != Z_STREAM_END) {
            QUP_LOGI("zlib inflate: zerr=%d (nIn=%p aIn=%u nOut=%p aOut=%u)\n",
                zerr, zstream.next_in, zstream.avail_in,
                zstream.next_out, zstream.avail_out);
            goto z_bail;
        }
            ...
            (zerr == Z_STREAM_END && zstream.avail_out != sizeof(writeBuf)))
        {
            ...
            if (cc < 0) {
                QUP_LOGI("write failed in inflate: %s", strerror(errno));
                goto z_bail;
            } else if (cc != (int) writeSize) {
                QUP_LOGI("write failed in inflate (%d vs %ld)", cc, writeSize);
                goto z_bail;
            }
